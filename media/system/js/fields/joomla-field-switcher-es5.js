...
(function (customElements) {
  ...
  function (_HTMLElement) {
    ...
    function JoomlaSwitcherElement() {
      ...
      _this = _possibleConstructorReturn(this, _getPrototypeOf(JoomlaSwitcherElement).call(this));
      _this.inputs = [];
      _this.spans = [];
      ...
      _this.newActive = '';
      _this.inputLabel = '';
      _this.inputLabelText = ''; // Let's bind some functions so we always have the same context
      ...
      _this.keyEvents = _this.keyEvents.bind(_assertThisInitialized(_this));
    ...
    _createClass(JoomlaSwitcherElement, [{
      ...
      value: function connectedCallback() {
        ...
        if (this.inputs[1].checked) {
          ...
          this.spans[1].classList.add('active'); // Aria-label ONLY in the container span!

          this.inputsContainer.setAttribute('aria-labelledby', "".concat(this.id, "-lbl")); // this.spans[1].innerHTML);
        } else {
          this.spans[0].classList.add('active'); // Aria-label ONLY in the container span!
      ...
      value: function createMarkup() {
        var checked = 0; // If no type has been defined, the default as "success"
        ...
        spanFirst.setAttribute('tabindex', '0'); // Set the id to the fieldset

        spanFirst.id = this.id; // Remove the id from the custom Element
        ...
        spanFirst.appendChild(switchEl); // Create the second 'span' wrapper
        ...
        spanSecond.appendChild(labelSecond); // Append everything back to the main element
      ...
      value: function _switch() {
        ...
        if (this.newActive === 1) {
          ...
          this.inputsContainer.setAttribute('aria-checked', true); // Aria-label ONLY in the container span!

          this.inputsContainer.setAttribute('aria-label', "".concat(this.inputLabelText, " ").concat(this.spans[1].innerHTML)); // Dispatch the "joomla.switcher.on" event
          ...
          this.inputsContainer.setAttribute('aria-checked', false); // Aria-label ONLY in the container span!

          this.inputsContainer.setAttribute('aria-label', "".concat(this.inputLabelText, " ").concat(this.spans[0].innerHTML)); // Dispatch the "joomla.switcher.off" event
