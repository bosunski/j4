...
(customElements => {
  ...
  class JoomlaSwitcherElement extends HTMLElement {
    ...
    constructor() {
      ...
      this.newActive = '';
      this.inputLabel = '';
      this.inputLabelText = ''; // Let's bind some functions so we always have the same context
    ...
    createMarkup() {
      let checked = 0; // If no type has been defined, the default as "success"
    ...
    switch() {
      ...
      if (this.newActive === 1) {
        ...
        this.inputsContainer.setAttribute('aria-label', `${this.inputLabelText} ${this.spans[1].innerHTML}`); // Dispatch the "joomla.switcher.on" event
        ...
        this.inputsContainer.setAttribute('aria-label', `${this.inputLabelText} ${this.spans[0].innerHTML}`); // Dispatch the "joomla.switcher.off" event
