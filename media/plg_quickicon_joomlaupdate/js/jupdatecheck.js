          ...
          if (Array.isArray(updateInfoList)) {
            if (updateInfoList.length === 0) {
              ...
              if (updateInfo.version !== options.version) {
                var messages = {
                  warning: ["<div class=\"message-alert\">\n  ".concat(Joomla.Text._('PLG_QUICKICON_JOOMLAUPDATE_UPDATEFOUND_MESSAGE').replace('%s', "<span class=\"badge badge-danger\"> \u200E ".concat(updateInfo.version, "</span>")), "\n  <button type=\"button\" class=\"btn btn-sm btn-primary\" onclick=\"document.location='").concat(options.url, "'\">\n    ").concat(Joomla.Text._('PLG_QUICKICON_JOOMLAUPDATE_UPDATEFOUND_BUTTON'), "\n  </button>\n</div>")]
                }; // Render the message

                Joomla.renderMessages(messages); // Scroll to page top

                window.scrollTo(0, 0);
                update('warning', Joomla.Text._('PLG_QUICKICON_JOOMLAUPDATE_UPDATEFOUND').replace('%s', "<span class=\"badge badge-light\"> \u200E ".concat(updateInfo.version, "</span>")));
              } else {
                update('success', Joomla.Text._('PLG_QUICKICON_JOOMLAUPDATE_UPTODATE'));
              }
            }
